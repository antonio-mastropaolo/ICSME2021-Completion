...
def extract_comments(code):
  ...
  for match in compiled.finditer(code):
    ...
    if kind == "single":
      ...
      indexes = match.span()
      comment = common.Comment(comment_content, line_no + 1, indexes[0], indexes[1])
      comments.append(comment)
    elif kind == "multi":
      comment_content = match.group("multi_content")
      indexes = match.span()
      comment = common.Comment(comment_content, line_no + 1, indexes[0], indexes[1], multiline=True)
      comments.append(comment)
    elif kind == "error":
